    ...
    <section className="py-16 sm:py-24 bg-brand-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-10 sm:mb-16">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-brand-darkgray mt-2">Step-by-Step: How It Works</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8 relative">
          ...
          {steps.map((step, index) => (
            <div key={index} className="bg-brand-white p-5 sm:p-8 rounded-xl shadow-soft border border-brand-lightgray hover:shadow-medium transition duration-300 relative z-10 group">
              <div className="w-12 h-12 sm:w-16 sm:h-16 bg-brand-teal/15 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6 mx-auto group-hover:bg-brand-blue group-hover:scale-110 transition-all duration-300">
              ...
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-blue text-brand-white w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center font-bold text-xs sm:text-sm border-4 border-brand-white">
                {index + 1}
              </div>
              <h3 className="text-lg sm:text-xl font-bold text-brand-darkgray mb-2 sm:mb-3 text-center">{step.title}</h3>
